...
%  objects     - a class of type CellGen_DynamicProps that defines the
%    different objects (nucleus, cytoplams).
%  markers     - a class of type CellGen_DynamicProps that defines two
%    different markers.
...
%    classes derived from CellGen_CellArtifact_Operation), used to
...
%%
  ...
  methods
    ...
    function obj=Subpopulation()
      obj.objects=CellGen_DynamicProps;%Shape_list;
      obj.markers=CellGen_DynamicProps;%Marker_list;
    end
    ...
    function add_object(obj,obj_name)
      ...
      %   is assumed to be a string) and type CellGen_Object in the objects property of the subpopulation specified by subpop.
      %
      %   See also  Subpopulation,CellGen_Object
      %
      %   Examples:
      ...
      obj.objects.(obj_name)=CellGen_Object();
    ...
    function add_marker(obj,marker_name,marker_color)
      ...
      %
      %   See also  Subpopulation,CellGen_Object,Marker
      %
      %   Examples:
      %   subpop=Subpopulation();
      %   add_object(subpop,'cytoplasm');
    ...
    function add_cell_artifact(obj,operation)
      ...
      %   by operation to the queue of CellGen_CellArtifact_Operation-s to
      %   be performed on cells in Subpopulation subpop.
      %
      %   See also  Subpopulation,CellGen_CellArtifact_Operation
      %
      %   Examples:
      %   subpop=Subpopulation();
      %   add_object(subpop,'cytoplasm');
  ...
  methods (Hidden)
    ...
    function [obvious_deletions,all_deletions]=calculate_all_dependancies(obj,to_be_deleted)
      ...
      if(isa(to_be_deleted,'CellGen_Object'))
    ...
    function [state,message,bg]=check_shape_dependancies(obj)
      ...
      if(S>1)
        ...
        try
          ...
          message={['CellGen requires all objects in a subpopulation be connected to each other (e.g. to place the nucleus inside the cytoplasm, one may pick cytoplasm is ''' 'centered around' ''' the nucleus).'], 'However there is at least one object that is not connected to the others. Please consider changing your object definitions to fix this'};
        catch
          message={['CellGen requires all objects in a subpopulation be connected to each other (e.g. to place the nucleus inside the cytoplasm, one may pick cytoplasm is ''' 'centered around' ''' the nucleus).']};
